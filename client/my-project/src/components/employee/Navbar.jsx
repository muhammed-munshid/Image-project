...
import LeaveRequestModal from './pages/LeaveRequestModal';
  ...
  return (
    <>
      <nav className="bg-red-600 p-3 flex justify-between items-center sticky top-0 h-16">
        ...
        <div className="flex items-center space-x-4">
          <button onClick={openModal} className="text-white bg-red-700 px-4 py-2 rounded hover:bg-red-800">Apply for Leave</button>
          ...
          <button onClick={handleLogout} className="text-white bg-red-700 px-4 py-2 rounded hover:bg-red-800">Logout</button>
        </div>
      </nav>
      {sidebarOpen && (
      ...
      <LeaveRequestModal isOpen={isModalOpen} onClose={closeModal} />
    </>
  );
}
